...
def grate_stage(things_to_place):
    ...
    product_to_grate = ProductToGrate(st[0] + '.png', int(st[1]), int(st[2]), int(st[3]), int(st[4]),
                                      (int(st[5]), int(st[6])), int(st[7]), int(st[8]), int(st[9]), st[10] + '.png',
                                      (int(st[11]), int(st[12])), (int(st[13]), int(st[14])))
...
def oil_stage(things_to_place):
    ...
    st = things_to_place[0].split()
    recepy = font.render('Намаж маслом курицу', 1, (255, 0, 0))
    recepy_rect = recepy.get_rect()
    recepy_rect.x = 10
    recepy_rect.y = 5
        ...
        for i in range(len(stage)):
            ...
            if stage[i] == 'grate':
                if not grate_stage(things_to_place[i]):
                    break
            if stage[i] == 'oil':
                if not oil_stage(things_to_place[i]):
                    break
            if stage[i] == 'oven':
